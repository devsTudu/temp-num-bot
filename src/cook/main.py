...
API_KEY = secrets.COOK_API_TOKEN
COOK_URL = "https://fastapi-tempnumbot.onrender.com"
MENU_LIST = "listOfServices.txt"
...
class serviceOperation:
    def __init__(self, file_address: str = MENU_LIST):
        ...
        def updatePages():
            logger.log(1,"Creating the pages for bot")
            menu = self.database
            service_list = []
            for i in menu:
            ...
            for i in range(num_chunks):
                ...
                with open(f"templates/page{i+1}.txt", "w") as f:
                    f.write("\n".join(data_to_load))
        ...
        if not path.isfile(file_address):
            ...
            updateAllDetails()
            updatePages()
        elif not path.isfile('templates/page1.txt'):
